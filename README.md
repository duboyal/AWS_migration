...
#### 3. [step function state machine](https://github.com/duboyal/AWS_migration/blob/main/my-infrastructure/stepfunction/stepfunction_statemachine.yaml) launches fargate task .The Fargate task needs to be launched in an ECS cluster. The ecs-cluster.yaml file defines the ECS cluster that the Fargate task will be launched in and should be linked. *NOTE* while the IAM roles are used to provide access control, they are not the primary mechanism used to link the ECS cluster and Fargate task. Instead, the ECS cluster provides the infrastructure and management layer for the Fargate tasks to run on. The IAM roles are used to give the task the necessary permissions to access other resources, such as S3 buckets or databases.
