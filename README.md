...
I would set up an s3 bucket that could take files and send rows from the csv over in "batches" (like 10 rows would be a batch) from the files to SQS queue events that would then send these "batches" to an instance of lamda fucnction (or lambda handler) to conduct the transforming the data. I would design a python class framework so the batches could take lines from a csv file , so 10 lines for an example batch of size 10. All these instances would save to a parquet file to a "transformed" s3 bucket that has a glue crawler attached to it where the glue crawler would be set up to read data from multiple files within the transformed s3 bucket . the file format of parquet is especially useful here because it is the safest for when multiple functions need to add to a file at once .
